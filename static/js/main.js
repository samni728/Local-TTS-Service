document.addEventListener('DOMContentLoaded', () => {
    ...
    let allVoices = [];
    let currentConfig = {};
    const OPENAI_VOICE_ALIASES = ["shimmer", "alloy", "fable", "onyx", "nova", "echo"];
    ...
    const mappingsContainer = document.getElementById('openai-mappings-container');
    ...
    const updateApiExamples = () => {
        const origin = window.location.origin; // 获取当前页面的源地址，例如 https://tts.123go.com 或 http://localhost:5050
...
-d '{
  ...
  "voice": "en-US-AriaNeural"
    ...
    const handleSaveSettings = async () => {
        ...
        OPENAI_VOICE_ALIASES.forEach(alias => {
            newConfig.openai_voice_map[alias] = document.getElementById(`mapping-${alias}`).value;
        });
        ...
        try {
            ...
            showMessage(settingsFeedback, result.message || result.error, !response.ok);
            if (response.ok) currentConfig.openai_voice_map = newConfig.openai_voice_map;
        } catch (error) {
    ...
    const handleSpeakClick = async () => {
        ...
        if (!text) { showMessage(errorMessage, "请输入文本内容。", true); return; }
        if (!voice || voiceSelect.disabled) { showMessage(errorMessage, "请选择一个有效的声音。", true); return; }
    ...
    const initialize = async () => {
        try {
            const [voicesResponse, configResponse] = await Promise.all([
                fetch('/v1/audio/all_voices'), fetch('/v1/config')
            ]);
            if (!voicesResponse.ok || !configResponse.ok) throw new Error("无法加载初始化数据。");
            
            allVoices = await voicesResponse.json();
            currentConfig = await configResponse.json();
            
            languageSelect.addEventListener('change', () => updateVoiceList(languageSelect.value));
            speakButton.addEventListener('click', handleSpeakClick);
            saveSettingsButton.addEventListener('click', handleSaveSettings);

            const defaultLang = Object.keys(currentConfig.openai_voice_map).map(k => currentConfig.openai_voice_map[k].substring(0, 5)).find(l => l.startsWith('zh')) || "zh-CN";
            languageSelect.value = defaultLang;
            updateVoiceList(defaultLang);
            populateSettingsForm();
            
            // NEW: 页面加载后立即更新 API 示例
            updateApiExamples();
